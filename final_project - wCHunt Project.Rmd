...
The derivative of $a(t)$ is the sum of two terms, $k_1Re^{-k_1t}$ and $-k_2a(t)$. If $a'(t) = u(t) \bullet v'(t) + v(t) \bullet u'(t)$
...
Exercise 3. If k_1 = 2 and k_2 = 1, how much food must the abomasum be able to hold if a meal of amount R is fed at time t = 0?
...
Equations (4) and (9) are purly theoretical and are based on assumptions. To confirm their accuracy we will see if they agree with experimental data. Since the data concern is fecal excretion as a function of time, we must first translate our results into results on fecal excretion. 
...
answer: $a'(t) > 0$ for all $t <t_0$; $a'(t_0) =0$; $a'(t) < 0$ fpr $t > t_0$. 

(b) Determine the values of t for which a''(t) is, respectively, positive, negative, and zero.

answer: $a''(t) < 0$ for all $t < 2t_0$; $a''(2t_0) =0$; $a''(t) > 0$ fpr $t > 2t_0$. 
...
Using all the information in Exercise 4, 5, 6, and 7 to sketch the graphs of r, a, d, and f. You should not hvae to compute any points except r(0), a(0), d(0), and f(T), which you already know. 
...
Assuming that we are looking for the feed type that stays in the sheeps stomach the longest, then we would say that the medium-ground grass would be the most desirable. We make this assumption as it stands that the sheep requires less feed for the same results. 
